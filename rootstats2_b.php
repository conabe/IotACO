    ...
    while($row = $result->fetch_assoc()) {
          ...
          if($hora != $hora_old){
           ...
           if($row['ip']==$iparray[0]){
              ...
              }else if($row['ip']==$iparray[1]){
              $example_data[]=array($hora_aux,'',$row['temp'],'','','','');
              }else if($row['ip']==$iparray[2]){
              $example_data[]=array($hora_aux,'','',$row['temp'],'','','');
              }else if($row['ip']==$iparray[3]){
              $example_data[]=array($hora_aux,'','','',$row['temp'],'','');
              }else if($row['ip']==$iparray[4]){
              $example_data[]=array($hora_aux,'','','','',$row['temp'],'');
              }else if($row['ip']==$iparray[5]){
              $example_data[]=array($hora_aux,'','','','','',$row['temp']);
              }
          }
    }
...
if($logado=='root'){
    ...
    for($i = 0; $i < count($iparray) ; $i++){
        $array_aux[] = long2ip($iparray[$i]);
    }
   $plot->SetLegend($array_aux);
